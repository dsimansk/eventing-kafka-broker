...
// kedacore/keda's go.mod require's k8s.io/* at v0.36.0 but replace's them back
// down to v0.35.x (and controller-runtime to v0.23.3). Replace directives don't
// apply transitively, so we mirror those pins here to keep this module on
// Kubernetes 1.35 rather than being dragged up to 0.36.x by keda's require's.
replace (
